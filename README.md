# Bazurto
Bazurto is a command to modify your environment and execute other commands distributed via Github.  The name Bazurto is a famous
...
The install script is being worked on and it has not been released yet
...
1. Download the binary for your operating system from latest release at https://github.com/bazurto/bz/releases/latest
2. Rename the executable to `bz` (or `bz.exe` if you are using Windows). If you are using Linux or Mac, make sure it has
    executable permissions (chmod +x bz).
3. Place the executable file in your PATH. For Linux or Mac a common directory to drop executables is `/usr/local/bin`.
    On Windows add the directory holding `bz.exe` to your PATH (or place it under a directory already in PATH).
4. You can now use the `bz` command.
...
- It caches the dependencies to user's home directory
...
1. Create project directory `mkdir example-package` and `cd example-package`
...
2. It looked for the first match against any defined aliases in all `.bz.hcl/.bz.lock` files (including the local one).
...
Aliases are platform and OS agnostic to distribute your commands.  Aliases are the easiest way to write software and 
distribute to other operating systems.
The downside of aliases is that you have to always call them with the `bz` command
and you cannot use them with new shells.
...
not be portable between Windows, Mac and Linux.  For this reason aliases are the most portable alternative.

If you need to provide binaries for different operating systems, place the executable in the `bin` directory.  Make sure you create a package
for each operating system you plan to support.

For example, for the `example-package` and release v1.0.0.  If you want to support Linux, Windows and Mac for AMD64/Intel architecture,
then you need to create the following packages with the corresponding executables in the `bin` directory.

- example-package-windows-amd64-v1.0.0.tgz
- example-package-linux-amd64-v1.0.0.tgz
- example-package-darwin-amd64-v1.0.0.tgz

Note that you have to follow these specific naming conventions for your packages to be found by `bz`.  The command always looks for
an OS/Arch specific package, if not found then it looks for a plain package.
...
You can find examples on how the python package is put together at [github.com/bazurto/python](https://github.com/bazurto/python)



### Available Variables:

For every dependency, the `bz` command defines a `_DIR` and a `_BINDIR` variable.  These two variables point to
where the cached dependency is uncompressed and the directory where binary executables are found.   It creates multiple
...
For the local package being developed, the DIR and BINDIR variables are defined. DIR points to the project directory and the
BINDIR defaults to "bin".   BINDIR can be overridden by setting `binDir` in `bz.hcl`.  e.g.:  `binDir = "$DIR/other"`.   Make sure to
use the variable `$DIR` when you set `binDir` so it points to the full location of your project.
...
To define a custom variable, set the `env` map in the `.bz.hcl` file. e.g.:
...
Variables defined in this section will be available to any other package requiring this project.  They can also be used in `binDir` sections, `alias` sections and other variables.
